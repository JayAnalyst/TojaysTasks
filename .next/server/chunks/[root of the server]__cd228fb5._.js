...
async function POST(req) {
    ...
    try {
        ...
        if (type === "team" && !teamMemberId) {
            return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
                error: "Team task must have an assigned team member"
            }, {
                status: 400
            });
        }
        ...
        console.log(`✅ Task created: ${task.title} (${task.type})`);
...
async function PUT(req) {
    try {
        ...
        console.log(`✅ Found task: ${task.title}, Type: ${task.type}, Completed: ${task.completed}`);
        ...
        // ✅ If it's a team task, update BOTH users' points
        if (task.type === "team" && task.teamMemberId) {
            console.log(`🔄 Updating points for Team Task. Users: ${task.userId}, ${task.teamMemberId}`);
            await prisma.user.updateMany({
                ...
                data: {
                    points: {
                        [completed ? "increment" : "decrement"]: task.points
                    }
                }
            });
            console.log(`✅ Points updated for both users`);
        } else {
            console.log(`🔄 Updating points for personal task, User: ${session.user.id}`);
            ...
            console.log(`✅ Points updated for user ${session.user.id}`);
        }
...
//# sourceMappingURL=%5Broot%20of%20the%20server%5D__cd228fb5._.js.map